...
def image_demo(input_path, output_path):
    ...
    for subdir in os.listdir(os.path.join(input_path)):
        ...
        for path, img, im0s, vid_cap, _ in dataset:
            ...
            if len(paths) != 5:
                for path in paths:
                    for track_finished in old_tracks_finished:
                ...
                pred_l = model_l(imgs, augment=False, visualize=False)
                pred_m = model_m(imgs, augment=False, visualize=False)
                ...
                for i, det in enumerate(zip(pred_l, pred_m)):
                    ...
                    bboxes = [convertBack(float(boxes[i][0]) * 1920 - 2, float(boxes[i][1]) * 1080 - 2,
                                          float(boxes[i][2]) * 1920 + 2, float(boxes[i][3]) * 1080 + 2, float(scores[i]),
                    ...
                    old_tracks_finished = tracks_finished
